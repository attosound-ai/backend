...
async fn main() -> std::io::Result<()> {
    ...
    if let Err(err) = app_logo_repo.ensure_indexes().await {
        log::error!("Failed to ensure app_logo indexes: {}", err);
    }
    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(chat_wallpaper_handler::admin_list_chat_wallpapers)
            .service(chat_wallpaper_handler::admin_create_chat_wallpaper)
            .service(chat_wallpaper_handler::admin_set_chat_wallpaper_active)
            .service(chat_wallpaper_handler::admin_delete_chat_wallpaper)
